...
makeU_I_rm <-function(cluster){
 revs <- reviews_filtered %>% filter(KMCluster = cluster)
   dplyr::select("username", "beer_id", "overall") 
 revs <- as.data.frame(revs)
 U_I_rm <- as(revs, "realRatingMatrix")
 return(paste0("U_I_rm",cluster))
 }
 #-------------------------------------------------------------------------------
U_I_rm_2 <- makeU_I_rm()
...
sim_mat <- similarity(U_I_Zsc, method = "Pearson")
...
```{r}
head(colCounts(U_I_rm))
summary(colCounts(U_I_rm))
hist(colCounts(U_I_rm), breaks = 100)

head(rowCounts(U_I_rm))
